...
- **Laptop fit (1366×768 is the target minimum)**: table cells use `px-2 py-3 2xl:px-3` and long text goes through `renderTruncated` (full text on hover), so every column including Flags stays visible. Chart axis labels shorten a word with "…" only when it would overlap a neighbour (`fitTickWords` in `utils/chartScale.js`). Check with `../dev_tools/ui_tour.py`.
